...
## 整体梳理
作为一个消息摘要算法，整个算法的核心思想是，输入的信息64字节为一组都“揉进”`uint32_t hash[8]`中，然后在将这个`hash` hex 编码。

关键的辅助函数`sha256_hash` `sha256_done`，`sha256_hash`负责将输入长度超出64字节的部分，分成64字节为一组进行加密，最后不足64字节不予处理。`sha256_done`负责处理最后不足64字节的部分，将它们按规则填充到64字节再加密，然后将`hash`转化输出。

核心加密函数`_hash`，将64字节加密，放到`hash`中。
