...
```
SHA256 的收尾工作, 总结一些就是如果缓冲区 `ctx->buf` 剩余的部分不足或正好为 8 字节(等同于 `ctx->len > 55`), 那么会进行两次 `_hash`: 
...
如果缓冲区 `ctx->buf` 剩余的部分大于 8 字节, 只会进行一次 `_hash`:
- 剩余 + 0x80 + 用 0 填充将缓冲区到 55 字节 + 最后 8 字节存放位计数器(`ctx->bits`)的低 32 位和高 32 位, 然后 `_hash`, 将信息“揉进” `ctx->hash` 中。
